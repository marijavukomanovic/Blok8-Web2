...
namespace WebApp.Controllers
{
    ...
    public class CenaKarteController : ApiController
    {
        ...
        public CenaKarteController(ICenaKarteRepository CenaKarteRepository, ICenovnikRepository cenovnikRepository, IKartaRepository kartaRepository, IKorisnikRepository korisnikRepository, ITipPutnikaRepository TipPutnikaRepository/*, IPayPalRepository PayPalRepository*/)
        // GET: api/CenaKartes
        {
        ...
        [Authorize(Roles = "AppUser")]
        [System.Web.Http.HttpGet]
        [Route("KupiKartuPayPal/{tipKarte}/{username}/{idPayPal}")]
        public async Task<IHttpActionResult> KupiKartuPayPal(int tipKarte, string username, int idPayPal)
        {
            lock (lockObj)
            {
                int idKorisnika = -1;
                ...
                /*foreach (var pay in payPalRepository.GetAll())
                {
                    if (pay.Id == idPayPal)
                    {
                        payPalId = pay.Id;
                ...
                PayPal pay = db.PayPall.Find(idPayPal);

                if (idKorisnika == -1)
                { return BadRequest("Ne postoji ili nije verifikovan korisnik"); }
